...
<Tip>
Use `computePaymentInfoHash()` from `@x402r/core` to compute the unique hash of a payment. Use `toPaymentInfo()` to deserialize a JSON PaymentInfo back into the typed struct.
</Tip>
...
const client = createPayerClient({ publicClient, walletClient, operatorAddress });

// Check when payment was authorized (requires escrow plugin)
...
```typescript
// Payer freezes payment (requires freeze plugin)
await client.freeze.freeze(paymentInfo);

// Merchant unfreezes payment (requires freeze plugin)
