...
* Create an arbiter client with the operator's deployment addresses
* Check for pending refund requests with `refund.has()` and `refund.get()`
* Review evidence with `evidence.getBatch()`
* Approve with `payment.refundInEscrow()` (recorder auto-approves)
* Deny with `refund.deny()` or decline with `refund.refuse()`
* Unfreeze payments with `freeze.unfreeze()`
* Distribute fees with `operator.distributeFees()`
...
* [Arbiter examples](https://github.com/BackTrackCo/x402r-sdk/tree/main/examples/arbiter)
