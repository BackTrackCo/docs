...
4. **RefundRequest (IHook)**: Wired as `voidPostActionHook`, auto-approves pending refund requests during `voidPayment()`
...
Call `deployMarketplaceOperator` from `@x402r/core` with a viem wallet client. Because every contract uses CREATE2, deploys are idempotent: re-running with the same parameters detects existing contracts and skips them.
...
For automated quality verification (AI garbage detection, schema validation), use the delivery protection preset. No RefundRequest, Evidence, or Freeze contracts. The arbiter or payer can capture funds, and the arbiter can issue immediate refunds without waiting for escrow expiry.
