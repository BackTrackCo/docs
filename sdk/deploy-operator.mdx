...
The fastest way to deploy an operator is to run the [example script](https://github.com/BackTrackCo/x402r-sdk/tree/main/examples/deploy-operator) directly.
...
This deploys a complete operator with 7-day escrow, 3-day freeze, and 1% operator fee. For quick E2E testing, use the short-escrow variant:

```bash
PRIVATE_KEY=0x... pnpm tsx examples/deploy-operator/deploy-short-escrow.ts
```

The script outputs all deployed contract addresses and BaseScan links for each transaction.
