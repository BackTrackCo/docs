...
* Node.js 18+ and npm
* An authorized payment on an x402r operator (see [Merchant Guide](/sdk/merchant))
...
<Note>
All payer actions (refund, freeze, evidence) must happen during the escrow period. Once escrow expires, the merchant can capture.
</Note>
...
<Warning>
`freeze()` blocks the merchant from releasing until the arbiter unfreezes. Only use when you need to prevent a capture during investigation.
</Warning>
...
<CardGroup cols={3}>
  <Card title="Arbiter Guide" icon="gavel" href="/sdk/arbiter">
