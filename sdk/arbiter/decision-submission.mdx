...
As an arbiter, you review pending refund requests, then either execute a refund via `payment.voidPayment` / `payment.refund` (which auto-approves the request), or terminally `refund.deny` / `refund.refuse` it. Evidence reads sit on `evidence.*`.

## Approve a refund (voidPayment)

To approve and execute a refund in one step, call `payment.voidPayment()`. This auto-approves the pending RefundRequest and transfers funds back to the payer.
...
<Warning>
`payment.voidPayment()` auto-approves the pending RefundRequest. There is no undo.
</Warning>
...
flowchart TD
    ...
    E --> F{Decision}
    F -->|Approve| G[payment.voidPayment]
