...
To approve and execute a refund, call `payment.refundInEscrow()`. This both auto-approves the pending RefundRequest and transfers funds back to the payer.

```typescript
const tx = await merchant.payment.refundInEscrow(paymentInfo, request!.amount)
