...
Use `createMerchantClient` for queries below; for executing a refund, see [Capture vs void](/sdk/merchant/quickstart#payment-voidpayment), the merchant client's `payment.voidPayment()` auto-flips the request to `Approved` through the `VOID_POST_ACTION_HOOK`.
