...
<Tip>
For accepting payments via HTTP, use x402's standard `paymentMiddleware` with the x402r facilitator service — see the [merchant-server example](/sdk/examples#merchant-server). This quickstart covers the `@x402r/merchant` SDK for post-payment lifecycle operations (release, charge, refund handling).
</Tip>
