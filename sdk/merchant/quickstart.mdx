...
// Or transform all routes
const routes = withRefund(existingRoutes, operatorAddress);
```
...
import { privateKeyToAccount } from 'viem/accounts';
import { X402rMerchant } from '@x402r/merchant';
import { PaymentState, RequestStatus, getNetworkConfig } from '@x402r/core';
...
async function main() {
  ...
  for (const hash of pending) {
    ...
    // Example: auto-approve small refunds
    if (details.maxAmount < BigInt('5000000')) { // < 5 USDC
      await merchant.approveRefundRequest(details);
      await merchant.refundInEscrow(details);
      console.log('Auto-approved small refund:', hash);
    }
  }
}
