...
### payment.refundInEscrow

Return escrowed funds to the payer before release.
...
### payment.approvePostEscrowRefund

Approve a post-escrow refund allowance. This sets how much the refund budget contract can pull from the merchant.

```typescript
const tx = await merchant.payment.approvePostEscrowRefund(paymentInfo, 5_000_000n)
console.log('Allowance approved:', tx)
```

### payment.getPostEscrowRefundAllowance

Check the current post-escrow refund allowance.

```typescript
const allowance = await merchant.payment.getPostEscrowRefundAllowance(paymentInfo)
