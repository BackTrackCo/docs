...
const refundableOption = refundable(
  ...
  operatorAddress,
  {
    ...
    refundRequestAddress: refundRequestAddress,
  }
);
...
// All payment options now include x402r metadata
```
...
const config = getNetworkConfig('eip155:84532'); // Base Sepolia
const operatorAddress = '0x...'; // Your PaymentOperator address
...
app.get('/api/data', (req, res) => {
  ...
  if (!payment) {
    return res.status(402).json({
      accepts: routes['/api/data'].paymentOptions,
    });
  }
