...
2. **Check Condition** - Call `condition.check(paymentInfo, amount, caller)`
...
| **Designated Address** | Any action per conditions (for example, `voidPayment()`, `capture()`, or `updateStatus()`) | Defined by StaticAddressCondition (arbiter, DAO, or service provider) |
