...
6. **Escrow period** begins (e.g., 7 days)
7. After escrow period: **Receiver** or **Arbiter** calls `operator.release(paymentId)`
8. **Operator** checks `RELEASE_CONDITION` (must pass escrow period check)
9. **Operator** calls `escrow.release()` to transfer funds to receiver
10. **Operator** distributes fees (if enabled)
...
3. **Arbiter** reviews dispute
4. **Arbiter** calls `refundRequest.updateStatus(paymentId, operator, Approved)`
5. **Arbiter** calls `operator.refundInEscrow(paymentId)`
6. **Operator** checks `REFUND_IN_ESCROW_CONDITION` (arbiter only)
...
- **Day 0-7:** Payer can freeze if suspicious
- **Day 3:** Payer freezes payment (freeze lasts 3 days)
- **Day 3-6:** Payment frozen, release blocked
- **Day 6:** Freeze expires automatically (or arbiter unfreezes early)
- **Day 7:** Escrow period ends
- **Day 7+:** Receiver or arbiter can release (if not frozen)
...
**OrCondition([ReceiverCondition, ArbiterCondition])**
...
Receiver Amount = 1000 - 0.5 = 999.5 USDC
```
...
| **Arbiter** | `refundInEscrow()`, `refundPostEscrow()`, `updateStatus()` on refund requests | Can only act on payments using their operator |
| **Owner** | `queueFeesEnabled()`, `executeFeesEnabled()`, `queueFeeUpdate()`, `executeFeeUpdate()`, `rescueETH()` | 24-hour timelock on fee changes |
