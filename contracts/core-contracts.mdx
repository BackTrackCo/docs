...
2. Check `REFUND_IN_ESCROW_CONDITION` (configurable per operator)
...
2. Check `REFUND_POST_ESCROW_CONDITION` (configurable per operator)
3. Call `escrow.reclaim()` to take back funds from receiver
4. Transfer to payer
5. Call `REFUND_POST_ESCROW_RECORDER`
6. Emit `PaymentRefunded`

**Access:** Controlled by `REFUND_POST_ESCROW_CONDITION`
...
uint8 public protocolFeePercentage;   // e.g., 25 = 25% to protocol, 75% to fee recipient
address public immutable FEE_RECIPIENT; // Who receives operator share of fees
```
...
```mermaid
flowchart TD
    Payment["Payment: 1000 USDC<br/>Max Fee: 5 bps (0.05%)<br/>Protocol %: 25%"]
    Payment --> TotalFee["Total Fee: 0.5 USDC"]
    TotalFee --> ProtocolFee["Protocol Fee: 0.125 USDC<br/>(25% of fee)"]
    TotalFee --> OperatorFee["Operator Fee: 0.375 USDC<br/>(75% of fee)"]
    ProtocolFee --> ProtocolRecipient[protocolFeeRecipient]
    OperatorFee --> FeeRecipient[FEE_RECIPIENT]
    Payment --> Receiver["Receiver Gets: 999.5 USDC"]

    style Payment fill:#4f46e5,stroke:#4338ca,color:#fff
    style TotalFee fill:#8b5cf6,stroke:#7c3aed,color:#fff
    style ProtocolFee fill:#f59e0b,stroke:#d97706,color:#fff
    style OperatorFee fill:#f59e0b,stroke:#d97706,color:#fff
    style Receiver fill:#10b981,stroke:#059669,color:#fff
```

**FEE_RECIPIENT examples:**
