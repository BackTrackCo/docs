...
3. **CAPTURE_PRE_ACTION_CONDITION** - Who can release from escrow
...
1. **AUTHORIZE_POST_ACTION_HOOK** - Record authorization (e.g., timestamp)
2. **CHARGE_POST_ACTION_HOOK** - Record charge event
3. **CAPTURE_POST_ACTION_HOOK** - Record release
...
- **Operator Fee:** 2 bps (0.02%) = 0.20 USDC -> goes to `FEE_RECEIVER`
