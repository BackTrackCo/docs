...
x402r adds escrow, refund windows, and dispute resolution on top of [Base Commerce Payments](https://github.com/coinbase/commerce-payments). This page shows the **measured gas cost** of every on-chain operation so you can evaluate the overhead.
...
| `distributeFees()` | 57,191 | 5.5x | Anyone | Batch across payments whenever you want |
...
An escrow authorization is inherently more work than a raw ERC-20 transfer (10,305 gas): it validates payment info, checks fee bounds, locks fees, transfers tokens into escrow, and records state. The per-plugin section below shows exactly where the gas goes.
