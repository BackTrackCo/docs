...
These operations only happen when a payment is disputed. Most payments never touch this path. The dispute process (freeze, evidence, arbiter approval) is fully on-chain — only the actual refund transfer requires the merchant's cooperation (via `ReceiverRefundCollector` approval) or funds still being in escrow.
