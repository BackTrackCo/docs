...
- Payment state machine: `NonExistent` → `InEscrow` → `Captured` → `Settled`
...
**Freeze** - Standalone condition that blocks capture when payment is frozen (with configurable freeze/unfreeze authorization)
