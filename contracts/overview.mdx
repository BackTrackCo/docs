...
## Commerce Payments <span style={{opacity: 0.6, fontSize: '0.8em'}}>(Base Layer)</span>

<Info>
The contracts in this section are part of [Base Commerce Payments](https://github.com/base/commerce-payments) (MIT licensed), not x402r. They provide the foundational escrow infrastructure that x402r extends.
</Info>

[Commerce Payments](https://github.com/base/commerce-payments) provides three core contracts:

**AuthCaptureEscrow** — Core escrow contract for holding ERC-20 tokens during payments. Manages the payment state machine (`NonExistent` → `InEscrow` → `Released` → `Settled`) with authorize, capture, void, and reclaim operations.

**ERC3009PaymentCollector** — Gasless payment collection via ERC-3009 `transferWithAuthorization`. Supports nonce-based replay protection, deadline-based expiry, and Multicall3 batching.

**TokenStore** — Safe token storage and transfer utilities with reentrancy protection and OpenZeppelin SafeERC20 integration.

See [Core Contracts](/contracts/core-contracts#authcaptureescrow) for architecture diagrams and details.
