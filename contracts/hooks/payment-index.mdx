...
PaymentIndexRecorderHook records a sequential index for each payment action, enabling multiple refund requests per payment. Each time `record()` is called, the index increments by 1.
