...
- **As a hook:** Records the `block.timestamp` when a payment is authorized
...
EscrowPeriod extends [AuthorizationTimeRecorderHook](/contracts/hooks/authorization-time) and adds `ICondition` implementation. You don't need to deploy AuthorizationTimeRecorderHook separately, use EscrowPeriod directly.
