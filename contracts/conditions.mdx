...
Standalone condition that blocks release when a payment is frozen. Manages freeze/unfreeze state with optional escrow period time constraint.

**Deployment:** Via FreezeFactory

**Architecture:**
- Implements `ICondition`
- Uses a `FreezePolicy` to determine who can freeze/unfreeze
...
Determines who can freeze/unfreeze payments. Delegates authorization to `ICondition` contracts.
...
function canFreeze(
    AuthCaptureEscrow.PaymentInfo calldata paymentInfo,
    address caller
) external view returns (bool allowed, uint256 duration);
...
```typescript
// Payer can freeze, arbiter can unfreeze, 3-day freeze duration
...
// Then deploy Freeze condition using this policy
const freeze = await freezeFactory.deploy(
  freezePolicy,         // freeze policy address
