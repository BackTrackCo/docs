...
```mermaid
graph LR
    Client -->|"immediate<br/>no recourse"| Receiver
...
```mermaid
graph LR
    Client -->|authorize| Escrow
    Escrow -->|"Operator Logic"| Receiver
    Escrow -.->|"Funds held until:<br/>• Operator calls capture()<br/>• Or payer reclaims after expiry"| Escrow

    style Client fill:#4f46e5,stroke:#4338ca,color:#fff
    style Escrow fill:#f59e0b,stroke:#d97706,color:#fff
    style Receiver fill:#10b981,stroke:#059669,color:#fff
```
