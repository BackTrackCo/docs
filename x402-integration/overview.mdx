...
sequenceDiagram
    ...
    Facilitator->>Escrow: authorize(paymentId, amount)
    Note over Escrow: Funds locked
    Facilitator-->>Server: Settlement confirmed
    Server-->>Client: 200 OK + resource

    Note over Server,Operator: Later: capture or void
    Server->>Operator: capture(paymentId)
    Operator->>Escrow: release(paymentId)
    Note over Escrow: Funds released to receiver
```
