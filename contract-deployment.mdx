...
description: "Deploy x402r smart contracts to EVM networks using Foundry deployment scripts"
icon: "rocket"
---
...
<Info>
New to x402r contracts? Start with [Smart Contracts](/contracts) to understand the architecture and factory patterns.
</Info>

<Warning>
Contracts are currently **UNAUDITED**. Use at your own risk. Owner addresses MUST be multisig wallets (e.g., Gnosis Safe) in production.
</Warning>
...
4. **ArbitrationOperatorFactory** - Factory for operator instances
...
<Tip>
The ArbitrationOperatorFactory uses CREATE2 for deterministic addresses. Calling with the same configuration returns the existing operator instead of deploying a new one.
</Tip>
...
- [ ] Monitoring systems ready (see [Monitoring](/monitoring))
...
forge verify-contract $CONTRACT_ADDRESS \
  src/commerce-payments/operator/ArbitrationOperatorFactory.sol:ArbitrationOperatorFactory \
  --chain-id 84532 \
  --watch
```
...
<CardGroup cols={2}>
  <Card title="Smart Contracts" icon="cube" href="/contracts">
    Learn about contract architecture and configuration examples.
  </Card>
  <Card title="Monitoring" icon="chart-line" href="/monitoring">
    Set up monitoring for deployed contracts and track payment activity.
  </Card>
  <Card title="Security" icon="shield" href="/security">
    Review security considerations and incident response procedures.
  </Card>
</CardGroup>
